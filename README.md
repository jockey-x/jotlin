...
- The chart supports using an existing Kubernetes `Secret` instead of generating one from Helm values
...
- If `secret.create=true`, then also replace all `secretEnv.*` placeholders
